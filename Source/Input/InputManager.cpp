...
struct Input
{
	...
	Array<KeyPressedFunc> keyPressedEvents;
	Array<KeyReleasedFunc> keyReleasedEvents;
	Array<KeyStateDesc> keyStateEvents;
	uint32 keyCode;
	uint32 endedDown : 1;
...
struct InputState
{
	Array<uint32> inputStateAccumulation[KeyState_Max];
...
static Map<KeyInput, Array<InputHoldDescription>> inputHoldMap;
...
{
	Array<InputHoldDescription>& holdFunctions = inputHoldMap[key];
...
Array<MouseSubscriber> mouseMovementSubscribers;
...
namespace Internal
{
void KeyMessageDownReceived(uint32 keyCode, bool isPressed, [[maybe_unused]] bool isRepeated)
{
...
void InputManager::SetContinuousInputFunction(KeyInput key, int32 stateMod, ContinuousInputFunction&& inputFunc)
{
	...
	if (!inputMap.TryFind(key, inputState))
	{
		...
		inputHoldMap.Add(key, Array<InputHoldDescription>{});
	}
