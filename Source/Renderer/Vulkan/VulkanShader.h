...
public:
	...
	void Compile(const Array<uint8>& source, ShaderStage stage);
...
private:
	...
	uint32 sourceSize = 0;
	ShaderStage shaderStage = Stage_Max;
};

struct VulkanShaders
{
	VulkanShader* shaders[Stage_Max];
	~VulkanShaders()
	{
		delete shaders[Stage_Vert];
		delete shaders[Stage_Frag];
	}
};