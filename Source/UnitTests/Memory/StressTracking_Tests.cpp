//-----------------------------------------------------------------------------
// Copyright Ed Keenan 2017
// Optimized C++
//----------------------------------------------------------------------------- 

#include "Framework/UnitTest.h"
#include "Memory/MemoryEngine.h"
#include "Dog.h"

//--------------------------------------------------------------------------
// MACROS Test
//--------------------------------------------------------------------------
#define TEST_4_BYTE(ptr)      ( ((unsigned int)ptr & 0x03) == 0 )
#define TEST_8_BYTE(ptr)      ( ((unsigned int)ptr & 0x07) == 0 )
#define TEST_16_BYTE(ptr)     ( ((unsigned int)ptr & 0x0F) == 0 )
#define TEST_32_BYTE(ptr)     ( ((unsigned int)ptr & 0x1F) == 0 )
#define TEST_64_BYTE(ptr)     ( ((unsigned int)ptr & 0x3F) == 0 )
#define TEST_128_BYTE(ptr)    ( ((unsigned int)ptr & 0x7F) == 0 )
#define TEST_256_BYTE(ptr)    ( ((unsigned int)ptr & 0xFF) == 0 )

//---------------------------------------------------------------------------
// Test Files:
// Single step throught, set break points, but do not modify it
//---------------------------------------------------------------------------
TEST(test1_trackCheck, stressTestTracking_group)
{
	// Initialize the memory system
	Memory::Initialize();

	// create heap
	Memory::Code memResult;
	Heap *heap_A;
	memResult = Memory::HeapCreate(heap_A, 100 * 1024, "HEAP_A");
	CHECK(memResult == Memory::OK);
	Heap *heap_B;
	memResult = Memory::HeapCreate(heap_B, 50 * 1024, "HEAP_B");
	CHECK(memResult == Memory::OK);
	Heap *heap_C;
	memResult = Memory::HeapCreate(heap_C, 500 * 1024, "HEAP_C");
	CHECK(memResult == Memory::OK);

	char *p0 = new(heap_A, Memory::Align::Byte_4, __FILE__, __LINE__) char[300];
	char *p1 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[200];
	char *p2 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[5 * 1024];
	char *p3 = new(heap_A, Memory::Align::Byte_4, __FILE__, __LINE__) char[60 * 1024];
	char *p4 = new(heap_C, Memory::Align::Byte_4, __FILE__, __LINE__) char[200 * 1024];
	char *p5 = new(heap_C, Memory::Align::Byte_4, __FILE__, __LINE__) char[8 * 1024];
	char *p6 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[15 * 1024];
	char *p7 = new(heap_A, Memory::Align::Byte_4, __FILE__, __LINE__) char[10 * 1024];
	char *p8 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[15 * 1024];
	char *p9 = new(heap_C, Memory::Align::Byte_4, __FILE__, __LINE__) char[7 * 1024];

	CHECK(TEST_4_BYTE(p0));
	CHECK(TEST_4_BYTE(p1));
	CHECK(TEST_4_BYTE(p2));
	CHECK(TEST_4_BYTE(p3));
	CHECK(TEST_4_BYTE(p4));
	CHECK(TEST_4_BYTE(p5));
	CHECK(TEST_4_BYTE(p6));
	CHECK(TEST_4_BYTE(p7));
	CHECK(TEST_4_BYTE(p8));
	CHECK(TEST_4_BYTE(p9));

	Memory::Info info;

	Memory::GetInfo(info);

	// CHECK the global memory
	CHECK(info.CurrAllocIndex == 10);
	CHECK(info.CurrBytesUsed == (5 + 60 + 200 + 8 + 15 + 10 + 15 + 7) * 1024 + (200 + 300));
	CHECK(info.CurrHeapCount == 3);
	CHECK(info.CurrNumAlloc == 10);
	CHECK(info.PeakBytesUsed == (5 + 60 + 200 + 8 + 15 + 10 + 15 + 7) * 1024 + (200 + 300));
	CHECK(info.PeakHeapCount == 3);
	CHECK(info.PeakNumAlloc == 10);

	// CHECK the memory of each HEAP
	Heap::Info HeapInfo;

	heap_A->GetInfo(HeapInfo);
	CHECK(HeapInfo.CurrBytesUsed == (60 + 10) * 1024 + 300);
	CHECK(HeapInfo.CurrNumAlloc == 3);
	CHECK(strcmp(HeapInfo.Name, "HEAP_A") == 0);
	CHECK(HeapInfo.PeakBytesUsed == (60 + 10) * 1024 + 300);
	CHECK(HeapInfo.PeakNumAlloc == 3);
	CHECK(HeapInfo.TotalSize == 100 * 1024);

	heap_B->GetInfo(HeapInfo);
	CHECK(HeapInfo.CurrBytesUsed == (5 + 15 + 15) * 1024 + 200);
	CHECK(HeapInfo.CurrNumAlloc == 4);
	CHECK(strcmp(HeapInfo.Name, "HEAP_B") == 0);
	CHECK(HeapInfo.PeakBytesUsed == (5 + 15 + 15) * 1024 + 200);
	CHECK(HeapInfo.PeakNumAlloc == 4);
	CHECK(HeapInfo.TotalSize == 50 * 1024);

	heap_C->GetInfo(HeapInfo);
	CHECK(HeapInfo.CurrBytesUsed == (200 + 8 + 7) * 1024);
	CHECK(HeapInfo.CurrNumAlloc == 3);
	CHECK(strcmp(HeapInfo.Name, "HEAP_C") == 0);
	CHECK(HeapInfo.PeakBytesUsed == (200 + 8 + 7) * 1024);
	CHECK(HeapInfo.PeakNumAlloc == 3);
	CHECK(HeapInfo.TotalSize == 500 * 1024);

#ifdef _DEBUG
	// walk the tracking block list per heap
	Block *Tcurr;
	Block *Tprev;

	// HEAP A-------------------------------------------------------------------
	Tprev = 0;
	Tcurr = heap_A->GetTrackingHead();
	CHECK(Tcurr != 0);

	CHECK(Tcurr->GetAllocIndex() == 8);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetHeapNext() != 0);
	CHECK(Tcurr->GetHeapPrev() == 0);
	CHECK(Tcurr->GetLineNum() == 49);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetHeapNext();

	CHECK(Tcurr->GetAllocIndex() == 4);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetHeapNext() != 0);
	CHECK(Tcurr->GetHeapPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 45);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetHeapNext();

	CHECK(Tcurr->GetAllocIndex() == 1);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetHeapNext() == 0);
	CHECK(Tcurr->GetHeapPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 42);

	// HEAP B-------------------------------------------------------------------
	Tprev = 0;
	Tcurr = heap_B->GetTrackingHead();
	CHECK(Tcurr != 0);

	CHECK(Tcurr->GetAllocIndex() == 9);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetHeapNext() != 0);
	CHECK(Tcurr->GetHeapPrev() == 0);
	CHECK(Tcurr->GetLineNum() == 50);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetHeapNext();

	CHECK(Tcurr->GetAllocIndex() == 7);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetHeapNext() != 0);
	CHECK(Tcurr->GetHeapPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 48);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetHeapNext();

	CHECK(Tcurr->GetAllocIndex() == 3);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetHeapNext() != 0);
	CHECK(Tcurr->GetHeapPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 44);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetHeapNext();

	CHECK(Tcurr->GetAllocIndex() == 2);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetHeapNext() == 0);
	CHECK(Tcurr->GetHeapPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 43);

	// HEAP C-------------------------------------------------------------------
	Tprev = 0;
	Tcurr = heap_C->GetTrackingHead();
	CHECK(Tcurr != 0);

	CHECK(Tcurr->GetAllocIndex() == 10);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetHeapNext() != 0);
	CHECK(Tcurr->GetHeapPrev() == 0);
	CHECK(Tcurr->GetLineNum() == 51);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetHeapNext();

	CHECK(Tcurr->GetAllocIndex() == 6);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetHeapNext() != 0);
	CHECK(Tcurr->GetHeapPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 47);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetHeapNext();

	CHECK(Tcurr->GetAllocIndex() == 5);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetHeapNext() == 0);
	CHECK(Tcurr->GetHeapPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 46);
#endif // _DEBUG
}


TEST(test1_trackCheck2, stressTestTracking_group)
{
	// Initialize the memory system
	Memory::Initialize();

	// create heap
	Memory::Code memResult;
	Heap *heap_A;
	memResult = Memory::HeapCreate(heap_A, 100 * 1024, "HEAP_A");
	CHECK(memResult == Memory::OK);
	Heap *heap_B;
	memResult = Memory::HeapCreate(heap_B, 50 * 1024, "HEAP_B");
	CHECK(memResult == Memory::OK);
	Heap *heap_C;
	memResult = Memory::HeapCreate(heap_C, 500 * 1024, "HEAP_C");
	CHECK(memResult == Memory::OK);

	char *p0 = new(heap_A, Memory::Align::Byte_4, __FILE__, __LINE__) char[300];
	char *p1 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[200];
	char *p2 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[5 * 1024];
	char *p3 = new(heap_A, Memory::Align::Byte_4, __FILE__, __LINE__) char[60 * 1024];
	char *p4 = new(heap_C, Memory::Align::Byte_4, __FILE__, __LINE__) char[200 * 1024];
	char *p5 = new(heap_C, Memory::Align::Byte_4, __FILE__, __LINE__) char[8 * 1024];
	char *p6 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[15 * 1024];
	char *p7 = new(heap_A, Memory::Align::Byte_4, __FILE__, __LINE__) char[10 * 1024];
	char *p8 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[15 * 1024];
	char *p9 = new(heap_C, Memory::Align::Byte_4, __FILE__, __LINE__) char[7 * 1024];

	CHECK(TEST_4_BYTE(p0));
	CHECK(TEST_4_BYTE(p1));
	CHECK(TEST_4_BYTE(p2));
	CHECK(TEST_4_BYTE(p3));
	CHECK(TEST_4_BYTE(p4));
	CHECK(TEST_4_BYTE(p5));
	CHECK(TEST_4_BYTE(p6));
	CHECK(TEST_4_BYTE(p7));
	CHECK(TEST_4_BYTE(p8));
	CHECK(TEST_4_BYTE(p9));

	Memory::Info info;

	Memory::GetInfo(info);

	// CHECK the global memory
	CHECK(info.CurrAllocIndex == 10);
	CHECK(info.CurrBytesUsed == (5 + 60 + 200 + 8 + 15 + 10 + 15 + 7) * 1024 + (200 + 300));
	CHECK(info.CurrHeapCount == 3);
	CHECK(info.CurrNumAlloc == 10);
	CHECK(info.PeakBytesUsed == (5 + 60 + 200 + 8 + 15 + 10 + 15 + 7) * 1024 + (200 + 300));
	CHECK(info.PeakHeapCount == 3);
	CHECK(info.PeakNumAlloc == 10);

	// CHECK the memory of each HEAP
	Heap::Info HeapInfo;

	heap_A->GetInfo(HeapInfo);
	CHECK(HeapInfo.CurrBytesUsed == (60 + 10) * 1024 + 300);
	CHECK(HeapInfo.CurrNumAlloc == 3);
	CHECK(strcmp(HeapInfo.Name, "HEAP_A") == 0);
	CHECK(HeapInfo.PeakBytesUsed == (60 + 10) * 1024 + 300);
	CHECK(HeapInfo.PeakNumAlloc == 3);
	CHECK(HeapInfo.TotalSize == 100 * 1024);

	heap_B->GetInfo(HeapInfo);
	CHECK(HeapInfo.CurrBytesUsed == (5 + 15 + 15) * 1024 + 200);
	CHECK(HeapInfo.CurrNumAlloc == 4);
	CHECK(strcmp(HeapInfo.Name, "HEAP_B") == 0);
	CHECK(HeapInfo.PeakBytesUsed == (5 + 15 + 15) * 1024 + 200);
	CHECK(HeapInfo.PeakNumAlloc == 4);
	CHECK(HeapInfo.TotalSize == 50 * 1024);

	heap_C->GetInfo(HeapInfo);
	CHECK(HeapInfo.CurrBytesUsed == (200 + 8 + 7) * 1024);
	CHECK(HeapInfo.CurrNumAlloc == 3);
	CHECK(strcmp(HeapInfo.Name, "HEAP_C") == 0);
	CHECK(HeapInfo.PeakBytesUsed == (200 + 8 + 7) * 1024);
	CHECK(HeapInfo.PeakNumAlloc == 3);
	CHECK(HeapInfo.TotalSize == 500 * 1024);

#ifdef _DEBUG
	// walk the tracking block list per global list
	Block *Tcurr;
	Block *Tprev;

	// HEAP A-------------------------------------------------------------------
	Tprev = 0;
	Tcurr = Memory::DebugGetGlobalTrackingHead();
	CHECK(Tcurr != 0);

	CHECK(Tcurr->GetAllocIndex() == 10);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetGlobalNext() != 0);
	CHECK(Tcurr->GetGlobalPrev() == 0);
	CHECK(Tcurr->GetLineNum() == 234);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetGlobalNext();

	CHECK(Tcurr->GetAllocIndex() == 9);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetGlobalNext() != 0);
	CHECK(Tcurr->GetGlobalPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 233);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetGlobalNext();

	CHECK(Tcurr->GetAllocIndex() == 8);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetGlobalNext() != 0);
	CHECK(Tcurr->GetGlobalPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 232);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetGlobalNext();

	CHECK(Tcurr->GetAllocIndex() == 7);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetGlobalNext() != 0);
	CHECK(Tcurr->GetGlobalPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 231);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetGlobalNext();

	CHECK(Tcurr->GetAllocIndex() == 6);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetGlobalNext() != 0);
	CHECK(Tcurr->GetGlobalPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 230);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetGlobalNext();

	CHECK(Tcurr->GetAllocIndex() == 5);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetGlobalNext() != 0);
	CHECK(Tcurr->GetGlobalPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 229);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetGlobalNext();

	CHECK(Tcurr->GetAllocIndex() == 4);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetGlobalNext() != 0);
	CHECK(Tcurr->GetGlobalPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 228);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetGlobalNext();

	CHECK(Tcurr->GetAllocIndex() == 3);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetGlobalNext() != 0);
	CHECK(Tcurr->GetGlobalPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 227);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetGlobalNext();

	CHECK(Tcurr->GetAllocIndex() == 2);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetGlobalNext() != 0);
	CHECK(Tcurr->GetGlobalPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 226);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetGlobalNext();

	CHECK(Tcurr->GetAllocIndex() == 1);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetGlobalNext() == 0);
	CHECK(Tcurr->GetGlobalPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 225);
#endif // _DEBUG
}

TEST(test1_trackCheck3, stressTestTracking_group)
{
	// Initialize the memory system
	Memory::Initialize();

	Memory::Code memResult;
	// create heap
	Heap *heap_A;
	memResult = Memory::HeapCreate(heap_A, 100 * 1024, "HEAP_A");
	CHECK(memResult == Memory::OK);
	Heap *heap_B;
	memResult = Memory::HeapCreate(heap_B, 50 * 1024, "HEAP_B");
	CHECK(memResult == Memory::OK);
	Heap *heap_C;
	memResult = Memory::HeapCreate(heap_C, 500 * 1024, "HEAP_C");
	CHECK(memResult == Memory::OK);

	char *p0 = new(heap_A, Memory::Align::Byte_4, __FILE__, __LINE__) char[300];
	char *p1 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[200];
	char *p2 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[5 * 1024];
	char *p3 = new(heap_A, Memory::Align::Byte_4, __FILE__, __LINE__) char[60 * 1024];
	char *p4 = new(heap_C, Memory::Align::Byte_4, __FILE__, __LINE__) char[200 * 1024];
	char *p5 = new(heap_C, Memory::Align::Byte_4, __FILE__, __LINE__) char[8 * 1024];
	char *p6 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[15 * 1024];
	char *p7 = new(heap_A, Memory::Align::Byte_4, __FILE__, __LINE__) char[10 * 1024];
	char *p8 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[15 * 1024];
	char *p9 = new(heap_C, Memory::Align::Byte_4, __FILE__, __LINE__) char[7 * 1024];

	CHECK(TEST_4_BYTE(p0));
	CHECK(TEST_4_BYTE(p1));
	CHECK(TEST_4_BYTE(p2));
	CHECK(TEST_4_BYTE(p3));
	CHECK(TEST_4_BYTE(p4));
	CHECK(TEST_4_BYTE(p5));
	CHECK(TEST_4_BYTE(p6));
	CHECK(TEST_4_BYTE(p7));
	CHECK(TEST_4_BYTE(p8));
	CHECK(TEST_4_BYTE(p9));

	// delete in specified 
	delete p9;
	delete p3;
	delete p1;
	delete p8;
	delete p0;
	delete p6;
	delete p4;
	delete p5;

	Memory::Info info;

	Memory::GetInfo(info);

	// CHECK the global memory
	CHECK(info.CurrAllocIndex == 10);
	CHECK(info.CurrBytesUsed == (5 + 10) * 1024);
	CHECK(info.CurrHeapCount == 3);
	CHECK(info.CurrNumAlloc == 2);
	CHECK(info.PeakBytesUsed == (5 + 60 + 200 + 8 + 15 + 10 + 15 + 7) * 1024 + (200 + 300));
	CHECK(info.PeakHeapCount == 3);
	CHECK(info.PeakNumAlloc == 10);

	// CHECK the memory of each HEAP
	Heap::Info HeapInfo;

	heap_A->GetInfo(HeapInfo);
	CHECK(HeapInfo.CurrBytesUsed == (10) * 1024);
	CHECK(HeapInfo.CurrNumAlloc == 1);
	CHECK(strcmp(HeapInfo.Name, "HEAP_A") == 0);
	CHECK(HeapInfo.PeakBytesUsed == (60 + 10) * 1024 + 300);
	CHECK(HeapInfo.PeakNumAlloc == 3);
	CHECK(HeapInfo.TotalSize == 100 * 1024);

	heap_B->GetInfo(HeapInfo);
	CHECK(HeapInfo.CurrBytesUsed == (5) * 1024);
	CHECK(HeapInfo.CurrNumAlloc == 1);
	CHECK(strcmp(HeapInfo.Name, "HEAP_B") == 0);
	CHECK(HeapInfo.PeakBytesUsed == (5 + 15 + 15) * 1024 + 200);
	CHECK(HeapInfo.PeakNumAlloc == 4);
	CHECK(HeapInfo.TotalSize == 50 * 1024);

	heap_C->GetInfo(HeapInfo);
	CHECK(HeapInfo.CurrBytesUsed == 0);
	CHECK(HeapInfo.CurrNumAlloc == 0);
	CHECK(strcmp(HeapInfo.Name, "HEAP_C") == 0);
	CHECK(HeapInfo.PeakBytesUsed == (200 + 8 + 7) * 1024);
	CHECK(HeapInfo.PeakNumAlloc == 3);
	CHECK(HeapInfo.TotalSize == 500 * 1024);

#ifdef _DEBUG
	// walk the tracking block list per global list
	Block *Tcurr;
	Block *Tprev;

	// global -------------------------------------------------------------------
	Tprev = 0;
	Tcurr = Memory::DebugGetGlobalTrackingHead();
	CHECK(Tcurr != 0);

	CHECK(Tcurr->GetAllocIndex() == 8);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetGlobalNext() != 0);
	CHECK(Tcurr->GetGlobalPrev() == 0);
	CHECK(Tcurr->GetLineNum() == 410);

	Tprev = Tcurr;
	Tcurr = Tcurr->GetGlobalNext();

	CHECK(Tcurr->GetAllocIndex() == 3);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetGlobalNext() == 0);
	CHECK(Tcurr->GetGlobalPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 405);
#endif // _DEBUG

}

TEST(test1_trackCheck4, stressTestTracking_group)
{
	// Initialize the memory system
	Memory::Initialize();

	// create heap
	Memory::Code memResult;
	Heap *heap_A;
	memResult = Memory::HeapCreate(heap_A, 100 * 1024, "HEAP_A");
	CHECK(memResult == Memory::OK);
	Heap *heap_B;
	memResult = Memory::HeapCreate(heap_B, 50 * 1024, "HEAP_B");
	CHECK(memResult == Memory::OK);
	Heap *heap_C;
	memResult = Memory::HeapCreate(heap_C, 500 * 1024, "HEAP_C");
	CHECK(memResult == Memory::OK);

	char *p0 = new(heap_A, Memory::Align::Byte_4, __FILE__, __LINE__) char[300];
	char *p1 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[200];
	char *p2 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[5 * 1024];
	char *p3 = new(heap_A, Memory::Align::Byte_4, __FILE__, __LINE__) char[60 * 1024];
	char *p4 = new(heap_C, Memory::Align::Byte_4, __FILE__, __LINE__) char[200 * 1024];
	char *p5 = new(heap_C, Memory::Align::Byte_4, __FILE__, __LINE__) char[8 * 1024];
	char *p6 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[15 * 1024];
	char *p7 = new(heap_A, Memory::Align::Byte_4, __FILE__, __LINE__) char[10 * 1024];
	char *p8 = new(heap_B, Memory::Align::Byte_4, __FILE__, __LINE__) char[15 * 1024];
	char *p9 = new(heap_C, Memory::Align::Byte_4, __FILE__, __LINE__) char[7 * 1024];

	CHECK(TEST_4_BYTE(p0));
	CHECK(TEST_4_BYTE(p1));
	CHECK(TEST_4_BYTE(p2));
	CHECK(TEST_4_BYTE(p3));
	CHECK(TEST_4_BYTE(p4));
	CHECK(TEST_4_BYTE(p5));
	CHECK(TEST_4_BYTE(p6));
	CHECK(TEST_4_BYTE(p7));
	CHECK(TEST_4_BYTE(p8));
	CHECK(TEST_4_BYTE(p9));

	// delete in specified 
	delete p9;
	delete p3;
	delete p1;
	delete p8;
	delete p0;
	delete p6;
	delete p4;
	delete p5;

	Memory::Info info;

	Memory::GetInfo(info);

	// CHECK the global memory
	CHECK(info.CurrAllocIndex == 10);
	CHECK(info.CurrBytesUsed == (5 + 10) * 1024);
	CHECK(info.CurrHeapCount == 3);
	CHECK(info.CurrNumAlloc == 2);
	CHECK(info.PeakBytesUsed == (5 + 60 + 200 + 8 + 15 + 10 + 15 + 7) * 1024 + (200 + 300));
	CHECK(info.PeakHeapCount == 3);
	CHECK(info.PeakNumAlloc == 10);

	// CHECK the memory of each HEAP
	Heap::Info HeapInfo;

	heap_A->GetInfo(HeapInfo);
	CHECK(HeapInfo.CurrBytesUsed == (10) * 1024);
	CHECK(HeapInfo.CurrNumAlloc == 1);
	CHECK(strcmp(HeapInfo.Name, "HEAP_A") == 0);
	CHECK(HeapInfo.PeakBytesUsed == (60 + 10) * 1024 + 300);
	CHECK(HeapInfo.PeakNumAlloc == 3);
	CHECK(HeapInfo.TotalSize == 100 * 1024);

	heap_B->GetInfo(HeapInfo);
	CHECK(HeapInfo.CurrBytesUsed == (5) * 1024);
	CHECK(HeapInfo.CurrNumAlloc == 1);
	CHECK(strcmp(HeapInfo.Name, "HEAP_B") == 0);
	CHECK(HeapInfo.PeakBytesUsed == (5 + 15 + 15) * 1024 + 200);
	CHECK(HeapInfo.PeakNumAlloc == 4);
	CHECK(HeapInfo.TotalSize == 50 * 1024);

	heap_C->GetInfo(HeapInfo);
	CHECK(HeapInfo.CurrBytesUsed == 0);
	CHECK(HeapInfo.CurrNumAlloc == 0);
	CHECK(strcmp(HeapInfo.Name, "HEAP_C") == 0);
	CHECK(HeapInfo.PeakBytesUsed == (200 + 8 + 7) * 1024);
	CHECK(HeapInfo.PeakNumAlloc == 3);
	CHECK(HeapInfo.TotalSize == 500 * 1024);

#ifdef _DEBUG
	// walk the tracking block list per global list
	Block *Tcurr;
	Block *Tprev;

	// global -------------------------------------------------------------------
	Tprev = 0;
	Tcurr = heap_A->GetTrackingHead();
	CHECK(Tcurr != 0);

	CHECK(Tcurr->GetAllocIndex() == 8);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetHeapNext() == 0);
	CHECK(Tcurr->GetHeapPrev() == 0);
	CHECK(Tcurr->GetLineNum() == 527);

	Tprev = 0;
	Tcurr = heap_B->GetTrackingHead();
	CHECK(Tcurr != 0);

	CHECK(Tcurr->GetAllocIndex() == 3);
	CHECK(strcmp(Tcurr->GetFileName(), "stresstracking_tests.cpp") == 0);
	CHECK(Tcurr->GetHeapNext() == 0);
	CHECK(Tcurr->GetHeapPrev() == Tprev);
	CHECK(Tcurr->GetLineNum() == 522);

	Tprev = 0;
	Tcurr = heap_C->GetTrackingHead();
	CHECK(Tcurr == 0);
#endif // _DEBUG
}

// ---  End of File ---------------
